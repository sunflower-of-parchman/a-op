...
import type { WorkerRunSummary } from '../shared/httpService.ts'
...
export async function runDocumentWorker(
  ...
  while (processed + failed < maxJobs) {
    ...
    try {
      ...
      console.error(
        JSON.stringify({
          event: 'license-document-failed',
          licenseId: claim.license_id,
          errorCode,
        }),
      )
    } finally {
  ...
  const summary = { processed, failed }
  console.log(JSON.stringify({ event: 'license-document-worker-complete', ...summary }))
  return summary
}
