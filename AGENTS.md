...
For a repository-link request, `@sites` clones the repository, builds and privately hosts the complete neutral Site with fresh Sites-provided D1 and R2 resources, applies every migration, and verifies it before personalization.
...
Use the installed `@sites` workflow to clone, build, and host the repository. `npm run prepare:sites-release` verifies an exact release candidate from a clean `main` checkout when the workflow needs it. The Sites hosting helper owns Site creation, project linkage, packaging, version saving, and private deployment.
