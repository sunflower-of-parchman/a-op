...
INSERT INTO `__new_collection_tracks`("id", "collection_revision_id", "track_id", "track_revision_id", "position", "created_at") SELECT "id", "collection_revision_id", "track_id", "track_revision_id", "position", "created_at" FROM `collection_tracks`;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_release_tracks`("id", "release_revision_id", "track_id", "track_revision_id", "position", "disc_number", "track_number", "created_at") SELECT "id", "release_revision_id", "track_id", "track_revision_id", "position", "disc_number", "track_number", "created_at" FROM `release_tracks`;--> statement-breakpoint
...
CREATE TABLE `__new_media_derivatives` (
	...
	CONSTRAINT "media_derivatives_kind_valid" CHECK("__new_media_derivatives"."kind" in ('streaming', 'download', 'waveform', 'artwork', 'poster', 'thumbnail', 'transcript', 'document', 'other')),
...
CREATE TABLE `__new_media_jobs` (
	...
	CONSTRAINT "media_jobs_status_valid" CHECK("__new_media_jobs"."status" in ('pending', 'processing', 'ready', 'failed')),
	CONSTRAINT "media_jobs_derivative_kind_valid" CHECK("__new_media_jobs"."derivative_kind" in ('streaming', 'download', 'waveform', 'artwork', 'poster', 'thumbnail', 'transcript', 'document', 'other')),
	CONSTRAINT "media_jobs_attempt_count_nonnegative" CHECK("__new_media_jobs"."attempt_count" >= 0)
);
--> statement-breakpoint
INSERT INTO `__new_media_jobs`("id", "source_media_id", "derivative_kind", "processing_profile", "processing_version", "status", "requested_by_user_id", "lease_token", "lease_expires_at", "attempt_count", "result_derivative_id", "last_error_code", "last_operation_key", "created_at", "updated_at", "finished_at") SELECT "id", "source_media_id", "derivative_kind", "processing_profile", "processing_version", "status", "requested_by_user_id", "lease_token", "lease_expires_at", "attempt_count", "result_derivative_id", "last_error_code", "last_operation_key", "created_at", "updated_at", "finished_at" FROM `media_jobs`;--> statement-breakpoint
DROP TABLE `media_jobs`;--> statement-breakpoint
ALTER TABLE `__new_media_jobs` RENAME TO `media_jobs`;--> statement-breakpoint
