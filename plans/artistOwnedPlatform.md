...
- Observation: A safe evidence-only commit after candidate freeze intentionally makes the ordinary branch `HEAD` differ from the immutable runtime tag.
  Evidence: `build-week-hosted-candidate-20260715-121920` resolves to fully verified runtime commit `04f23fa4b8632b04609cd2689b3b575ec2b193b0`, while later commits update only evidence and operator documentation. The hosted runbook now requires build and deployment from a clean detached worktree at that tag and requires a new aggregate and tag for any runtime-affecting change.
