...
- [x] (2026-07-15) Complete the open-source license gate: Michael selected `AGPL-3.0-or-later`; the standard GNU license text, package metadata, README, ADR, and original demonstration-asset ledger agree. Publication remains separately approval-gated.
...
- [ ] Complete the approval-gated Milestone 4 hosted-worker proof (completed: one worker implementation, Docker image, local worker and container verification; remaining: deploy the worker and demonstrate hosted upload-to-`ready` after Michael explicitly approves that external action).
...
- [ ] Complete the approval-gated Milestone 5 Stripe sandbox proof (completed: official SDK integration, pinned API contract, signature tests, setup checks, mapping and recovery UI, and complete local simulation; remaining: connect an artist-owned Stripe test account and demonstrate one Checkout purchase, one membership, webhook replay, protected delivery, and customer portal cancellation after Michael explicitly approves that external action).
...
- [ ] Complete the approval-gated Milestone 6 Stripe sandbox proof (completed: exact selection snapshots, verified-event fulfillment, provider mappings, signature and replay tests, local checkout demonstration, private document delivery, and recovery; remaining: connect an artist-owned Stripe test account and demonstrate one real test-mode license Checkout and webhook after Michael explicitly approves that external action).
...
- [x] (2026-07-15 11:43Z) Completed all approval-independent Milestone 12 work: coherent fictional assets and provenance; screenshots; README, contribution, judge, and hosted-test documentation; deterministic `demo:local` and `demo:reset`; macOS Chromium/WebKit and Linux-CI Firefox coverage; one comprehensive clean-room rehearsal; and a second untouched two-command novice rehearsal that returned HTTP 200 and left the clone clean. Commits `aead7ab` and `0ab2c9e` contain the package.
...
- [x] (2026-07-15 12:22Z) Closed two completion-audit gaps: implemented optional Supabase OAuth as a typed, closed-default server-side PKCE flow with secure transaction cookies and a configured browser gate; and expanded the 15-job Linux workflow to cover catalog/media, commerce, dependency audit, the complete isolated browser regression, and Chromium/Firefox/WebKit. Commit `fdbd9fa` contains the implementation.
...
- [ ] Complete the remaining human and external gates: Michael selects the final public name; approves hosted resources; hosted Supabase advisors, Stripe test mode, deployed worker, reset, and all-browser CI pass; the final `/feedback` Session ID is confirmed; the video is recorded and approved; repository/judge access is approved; and Devpost submission is approved and performed.
...
- Observation: Playwright Firefox 151 reproducibly hangs before page creation on this macOS arm64 host with Mozilla's `RenderCompositorSWGL failed mapping default framebuffer` headless startup failure, even after a forced runtime replacement and under Node 24 and Node 25. Chromium and WebKit pass locally; the final Linux CI job keeps Firefox mandatory.
...
Milestone 4 implementation is complete locally. An owner or editor can manually author releases, tracks, credits, artwork, and collections; review private drafts; publish validated authored order atomically; request a durable signed TUS upload; and retry processing failures. Codex can inspect an approved media folder, produce a rights-gated editable proposal, apply it idempotently, and run the same worker library locally or from its Docker entrypoint to create metadata, preview, waveform, and optimized-artwork derivatives without changing the source. Visitors can navigate published releases, tracks, and collections and use one persistent accessible player; signed-in listeners can maintain isolated favorites, ordered playlists, and history. Evidence is the seven implementation commits from `bc89195` through `21476f6`, clean migration replay, database lint with no findings, `npm run verify:catalog`, production build, and passing desktop/mobile catalog, administration, upload, accessibility, and customer-isolation journeys. The only remaining Milestone 4 acceptance item is the explicitly approved external deployment of the prepared worker and a hosted upload-to-`ready` demonstration.

Milestone 5 implementation is complete locally. An owner can publish free, external, one-time, and recurring offerings; map Stripe products and prices without moving editorial authority out of the application; inspect redacted failures; and request an explicit verified-event replay. A customer can use the labeled local simulation to create durable Checkout intents, orders, memberships, refunds, and entitlements; retrieve protected downloads; inspect account history; and lose expired or revoked access without losing an unrelated permanent purchase. Evidence is commit `058201c`, clean schema lint, `npm run verify:commerce`, type checking, application lint, production build, secret scanning, and passing desktop/mobile commerce, account-isolation, owner-mapping, viewport, and accessibility journeys. The remaining Milestone 5 acceptance item is the explicitly approved connection of an artist-owned Stripe sandbox for one real test-mode purchase, membership, webhook replay, protected delivery, and portal cancellation demonstration.

Milestone 6 implementation is complete locally. An owner can publish complete, non-exclusive uses for a track, revise them without mutating prior versions, map the resulting offers for Stripe test mode, inspect issued licenses, and retry failed documents. A customer sees the exact terms and price before checkout, supplies the licensee and project facts, completes a labeled local purchase, receives one replay-safe issued license and private PDF in account history, and loses access after a full refund; unsupported uses enter an inquiry. Evidence is commit `d6991d2`, clean migration replay and schema lint, `npm run verify:licensing`, type checking, application lint, production build, browser-secret scanning, final two-page PDF extraction and visual inspection, and four passing desktop/mobile licensing journeys with two intentional shared-database skips. The remaining Milestone 6 acceptance item is one explicitly approved Stripe sandbox license Checkout and verified webhook demonstration.
...
The approval-independent Milestone 13 package is prepared. The repository includes a direct project description, full judging guide, redacted hosted plan, 2:55 narrated demo script, and submission checklist with every external action separated. The final project name, hosted results, Linux CI result, `/feedback` confirmation, recorded video timecodes, public access, and Devpost confirmation can only be completed through the remaining human or explicitly approved external actions.
...
Revision note, 2026-07-15: Completed all approval-independent Milestone 12 work with original fictional assets, screenshots, deterministic demo and reset commands, full judge and contribution documentation, Chromium/WebKit local coverage plus mandatory Firefox Linux CI configuration, and two clean-clone rehearsals. Commits `aead7ab` and `0ab2c9e` contain the package; final licensing and hosted proof remain explicit gates.
...
Revision note, 2026-07-15: Michael selected `AGPL-3.0-or-later` for the repository and its original fictional demonstration assets. Added GNU's standard license text and reconciled package metadata, README, architecture, asset provenance, completion audit, and submission checklist. Repository publication remains a separate approval.
...
Revision note, 2026-07-15: Closed the completion-audit OAuth and CI gaps in commit `fdbd9fa` with a closed-default Supabase PKCE implementation, configured and disabled browser gates, and 15 Linux jobs covering every local module, full browser regression, dependency audit, and Chromium/Firefox/WebKit package route; added the requirement-level completion audit; and passed the expanded full aggregate through 12 isolated browser specifications.
