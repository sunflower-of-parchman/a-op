...
- [ ] Complete the remaining human and external gates: Michael approves the hosted policy migration and, if already entitled without a plan or billing change, leaked-password protection; otherwise record the documented plan limitation. Michael separately approves the revised Vercel bootstrap contract, webhook and transactional proof, hosted worker and reset, Linux all-browser CI, final `/feedback` confirmation, video, repository/judge access, and Devpost submission.
