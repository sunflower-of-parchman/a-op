...
- [x] (2026-07-16 23:36Z) Expanded the public music browser in commit `7c912c1`: visitors can move among Tracks, Albums, Collections, and signed-in Playlists; search and filter tracks by supported catalog metadata; apply view-appropriate sorting; and start playback through the existing persistent player. The production build, integration and documentation checks, browser-secret scan, and focused desktop/mobile catalog and library journeys passed.
- [x] (2026-07-16 23:46Z) Extended the literal first-clone contract through track detail pages in commit `978c5d6`: starter mode now presents `Track Title`, `Track Description`, `Track Metadata`, `Favorites and Playlists`, and `Return to Album`; the poetic detail and library headings were removed from the shared page; and six desktop/mobile starter journeys, type checking, lint, 28 unit tests, documentation, production build, and live Chrome verification passed.
- [x] (2026-07-17 00:16Z) Refined the starter and public music experience in commit `eab8365`: pinned OFL-licensed Lato for the starter, added persistent light and dark modes, expanded the public library to Explore, Tracks, Collections, Albums, Favorites, and Playlists, completed meter/tempo/mood/key/instrument/duration filters, exposed sanitized published waveforms, connected real commerce/licensing/library actions, and collapsed mobile filters by default. Production build, typecheck, lint, formatting, documentation, browser-secret scanning, eight starter journeys, two catalog journeys, and live responsive visual checks passed.
- [x] (2026-07-17 00:38Z) Completed the public starter-language pass in commit `3d61f56`: every public route family now teaches its real content and action elements with literal labels, including releases, collections, About, Contact, Support, Licensing, Privacy, learning, video, journal, account, authentication, and local checkout. Shared structured-section and video rendering preserve functional links, media authority, access rules, playback, and commerce behavior while suppressing fictional copy in starter mode. Formatting, lint, type checking, production build, ten desktop/mobile Chromium journeys spanning seventeen route archetypes, and live About, Support, lesson, and mobile visual checks passed.
- [x] (2026-07-17 00:49Z) Removed public instructional chrome and redundant eyebrow labels in commit `9ec421a`: the starter notice bar is gone, every public route begins with its actual title, and the music sidebar begins directly with its views instead of repeating `Library`. Administration retains its utility breadcrumbs and editable eyebrow fields. Formatting, lint, type checking, production build, ten serialized desktop/mobile Chromium journeys spanning eighteen route archetypes, and live music-page inspection passed with zero notice bars, zero public eyebrow elements, and zero horizontal overflow.
...
- Observation: Text substitution assertions do not detect fictional language embedded in image assets or an unavailable protected-media placeholder.
  Evidence: Live lesson-page inspection exposed both conditions after the route copy was literal. Starter video rendering now suppresses the fictional demo poster, and starter lessons render a designed `Lesson Image or Notation` surface instead of requesting protected fixture media. The corrected lesson and complete route matrix then passed visual and automated checks.

- Observation: A desktop catalog rail becomes an obstacle on a narrow screen when every filter precedes the first result.
  Evidence: The first 390-pixel inspection placed the track results below the expanded six-filter rail. The final hydrated mobile layout keeps the same filter authority behind a keyboard-accessible disclosure, preserves zero horizontal overflow, and passed the desktop/mobile catalog and accessibility journeys.

- Observation: A capable music library feels substantially more useful when one catalog can be reorganized by task without inventing popularity data.
  Evidence: The public browser now uses the artist's authored order as its default, offers metadata-backed filters and deterministic sorts, and exposes releases, collections, and each listener's private playlists from their existing authorities. It does not manufacture play counts, rankings, or unsupported musical fields.
...
- Observation: A polished fictional artist is strong functional evidence and weak first-clone teaching copy when it occupies the artist's own future site.
  Evidence: Michael's live homepage and track-detail reviews identified the Daymark wordmark, poetic headline, fictional track title, and fictional description as content that looked prescribed rather than editable. The new local starter keeps the same typography, spacing, links, release surface, track metadata, actions, and responsive layout while literal labels name each decision; the finished Daymark fixture remains separately launchable and unchanged.
...
- Decision: Make the public music surface a catalog browser with Tracks, Albums, Collections, and Playlists, using artist-authored order and only metadata the platform actually owns.
  Rationale: These views represent distinct listener tasks while preserving one catalog, one playback authority, and private per-account playlist state. Honest metadata-backed filters are more reusable than demonstration-only rankings.
...
- Decision: Give an unpersonalized local clone a literal teaching scaffold while preserving Daymark Assembly as the explicit complete demonstration.
  Rationale: Every starter surface should help an artist and Codex name and replace real page elements without suggesting that the fictional artist's language is a template. A development-only presentation layer preserves the working database fixture and layout, follows project-state personalization, and is structurally unable to replace published artist content in production.
  Date/Author: 2026-07-16 / Michael and Codex

- Decision: Start the teaching scaffold in Lato with persistent light and dark modes, and present music as a dense working library.
  Rationale: A neutral sans-serif and equal color modes give the first clone a legible baseline that the artist can later replace. Dense rows, literal starter labels, supported metadata filters, and authority-backed actions teach the complete catalog system without prescribing an artist voice or introducing decorative controls.
  Date/Author: 2026-07-16 / Michael and Codex

- Decision: Treat starter mode as a complete public presentation boundary, including database-backed structured sections and accessible media metadata, without mutating the fictional demonstration fixtures.
  Rationale: A new artist should encounter instructional anatomy on every public route they can visit, while the same routes retain their real behavior and the explicit Daymark demonstration remains a complete proof of the personalized platform.
  Date/Author: 2026-07-16 / Michael and Codex

- Decision: Use literal starter placeholders only where they occupy actual artist content or explain a working action; do not add a global notice bar or eyebrow label above public page titles.
  Rationale: The composition should teach through the site itself. Page titles, navigation, counts, controls, and results already provide orientation, while additional commentary and repeated category labels compete with the artist's future hierarchy.
  Date/Author: 2026-07-16 / Michael and Codex
...
The public catalog now presents that same authority as a complete music browser. Tracks support instant search plus meter, tempo, mood, and instrument filters; each view has relevant deterministic sorting; albums and collections preserve their editorial relationships; signed-in listeners can reach their private ordered playlists; and every playable result enters the existing persistent player. Commit `7c912c1` and focused desktop/mobile catalog and library journeys provide the evidence without introducing a second player, fabricated statistics, private reference content, or new provider state.
...
The first-clone experience now begins with the product's actual visual composition and literal names for each artist-editable element. A new artist can run `npm run starter:local`, see where their name or logo, headline, introduction, actions, release, supporting material, footer, track title, track description, metadata, listener library, and album return action will live, and discuss those decisions with Codex before applying content. The complete Daymark Assembly installation remains available through `npm run demo:local`, and production builds always use authoritative artist content. Commits `dbfc659` and `978c5d6`, six desktop/mobile starter journeys, the unchanged four-journey Daymark regression, live Chrome inspection, and the production-refusal probe provide the evidence.

The starter now has a locally bundled Lato baseline and persistent light/dark choice, while the public music page behaves as a working catalog instead of an editorial introduction. Six views, six supported filter families, deterministic sorting, public waveform rendering, purchase and license routes, favorites, playlists, responsive disclosure, and the persistent player share one cardless library surface. Commit `eab8365`, the production build, eight starter journeys, two desktop/mobile catalog journeys, and live light/dark/mobile inspection provide the evidence. Sound for Movement remained a read-only reference and no provider, database, deployment, or private resource changed.

The instructional first-clone language now covers the complete public application rather than only its home, music index, and track detail. A new artist can inspect releases, collections, biography, contact, support, licensing, privacy, courses, lessons, videos, journal entries, account, sign-in, sign-up, and simulated checkout while each surface names its actual content and action elements. Commit `3d61f56`, ten desktop/mobile starter journeys spanning seventeen route archetypes, and live About, Support, lesson, and mobile inspection prove the result. The implementation preserves real route behavior and Daymark fixtures without changing Supabase, provider, deployment, publication, or Sound for Movement state.

The public starter now teaches through the artist's actual composition rather than through explanatory chrome. Every public route begins with its real page title, and the music workspace moves directly from `Music` into views, filters, results, sorting, and track actions. Commit `9ec421a`, the production build, ten desktop/mobile Chromium journeys spanning eighteen route archetypes, and live inspection proving zero notice bars, zero public eyebrows, and zero overflow establish the result. Administration retains the utility breadcrumbs and editable eyebrow fields needed to operate authored content.
...
Revision note, 2026-07-16: Michael's track-detail review extended the same first-clone rule beyond the homepage. Commit `978c5d6` replaces the fictional track title, fictional description, poetic detail heading, poetic library heading, and release-specific return label with literal element or action names in starter mode while preserving the complete Daymark demonstration. Six desktop/mobile starter journeys and a live Chrome check prove the old language is absent. No database, provider, deployment, publication, or Sound for Movement state changed.

Revision note, 2026-07-16: Michael selected Lato as the starter typeface, requested equal light and dark presentation, and asked for the music browser to carry the operational density of the read-only Sound for Movement reference. Commit `eab8365` implements those choices independently with six catalog views, six supported filter families, public waveform sanitization, authority-backed row actions, persistent color mode, and a collapsed mobile filter disclosure. Production build, typecheck, lint, formatting, eight starter journeys, two catalog journeys, and live responsive inspection passed. No Supabase, provider, deployment, publication, or Sound for Movement state changed.
