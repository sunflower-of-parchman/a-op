...
The approval-independent Milestone 13 package is prepared. The repository includes a direct project description, full judging guide, redacted hosted plan, 2:55 narrated demo script, and submission checklist with every external action separated. The final project name, license, hosted results, Linux CI result, `/feedback` confirmation, recorded video timecodes, public access, and Devpost confirmation can only be completed through the remaining human or explicitly approved external actions.
