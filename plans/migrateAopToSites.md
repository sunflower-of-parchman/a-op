...
- [x] (2026-07-21 one-prompt sequence) Corrected the repository-owned ChatGPT Work handoff so the exact two-line `@Sites` repository prompt builds, privately deploys, and verifies the complete neutral installation before any capability, content, asset, design, or setup question. Moved capability selection into the post-deployment personalization proposal and aligned `AGENTS.md`, `README.md`, `SETUP.md`, the Sites release contract, and its focused contract test.
