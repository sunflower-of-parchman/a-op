...
- [ ] (2026-07-19 stopping point) Began the approved local judge-content rehearsal packet with a bounded four-release, 25-track, ten-post, two-Course, five-video, two-update, About, and four-hero manifest. Added a read-only source/local-copy builder, checksum verifier, activation interview, acceptance journey, and ignored packet destination. Packet generation, public-page hero integration, and setup-topic expansion remain for the fresh task. No production write, customer record, credential, or remote state is involved.
- [ ] (2026-07-20 release audit) Completed the read-only repository and release-boundary audit and rewrote `README.md` around the actual public, customer, administration, setup, and current-release surfaces. Type checking, formatting, the production build with fictional test credentials, and the production dependency audit passed. The final complete source test sweep passed 609 of 632 checks. Release remains blocked by 23 stale or failing contracts, unwired approved page heroes, unverified one-shot installer scripts, local judge media entering `dist/client`, the resulting client-size gate failure, and the real Stripe Test Checkout/provider-webhook rehearsal. No file was deleted.
...
- Observation: Ignoring local judge media in Git does not keep it out of a Sites build.
  Evidence: The current ignored `public/judge-content/` directory is copied into `dist/client/judge-content/` by the production build. The resulting client output is about 1.6 GB and fails the 1.5 MB client artifact budget. Packaging the current `dist/` would therefore carry local Sound for Movement rehearsal material.
...
- Observation: The existing setup contract can publish catalog, Courses, and Videos, while the judge rehearsal also needs first-class setup topics for Posts, What's New, About, and public hero media.
...
- Decision: Keep the judge-content packet local and ignored while tracking only its deterministic builder, verifier, manifest contract, and rehearsal documentation.
  Rationale: Sound for Movement material is Michael-approved for the local competition rehearsal, while the neutral open-source repository remains free of copied artist content and machine-specific generated bytes.
