...
import { publishEditorialPost } from "@/db/editorial-write.ts";
import {
  readJsonMutation,
...
export async function POST(
  ...
  return runApiRoute("admin.editorial_publish_failed", async (requestId) => {
    const requestInput = await readJsonMutation(request);
    const idempotencyKey = requireIdempotencyKey(request);
    const input = requireMutationObject(
      requestInput,
      ["expectedRevision"],
      "Editorial publication request",
    );
    ...
    const result = await publishEditorialPost(env.DB, slug, expectedRevision, {
