...
import { publishContentSection } from "@/db/content-section-write.ts";
import {
  readJsonMutation,
...
export async function POST(
  ...
  return runApiRoute(
    "admin.content_section_publish_failed",
    async (requestId) => {
      ...
      const input = requireMutationObject(
        requestInput,
        ["expectedVersion"],
        "Content section publication request",
      );
      ...
      const result = await publishContentSection(
        env.DB,
        keyResult.value,
        expectedVersion,
        {
