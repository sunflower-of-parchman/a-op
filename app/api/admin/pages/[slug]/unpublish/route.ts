...
import { unpublishPage } from "@/db/page-write.ts";
import {
  readJsonMutation,
...
interface PageUnpublishRouteContext {
  readonly params: Promise<{ slug: string }>;
}

export async function POST(
  request: Request,
  context: PageUnpublishRouteContext,
): Promise<Response> {
  return runApiRoute("admin.page_unpublish_failed", async (requestId) => {
    const requestInput = await readJsonMutation(request);
    const idempotencyKey = requireIdempotencyKey(request);
    const input = requireMutationObject(
      requestInput,
      ["expectedVersion"],
      "Page unpublication request",
    );
    ...
    const result = await unpublishPage(env.DB, slug, expectedVersion, {
