...
import { activateCommerceProduct } from "@/db/commerce-admin-write.ts";
import {
  readJsonMutation,
...
export async function POST(
  ...
  return runApiRoute(
    "admin.commerce_product_activate_failed",
    async (requestId) => {
      const requestInput = await readJsonMutation(request);
      const idempotencyKey = requireIdempotencyKey(request);
      const input = requireMutationObject(
        requestInput,
        ["expectedRevision", "licenseOffer"],
        "Commerce-product activation request",
      );
      ...
      const result = await activateCommerceProduct(
        env.DB,
